...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ═══════════════════════════════════════════════════════════
  // LAYER 2: THREE.JS & CANVAS AMBIENT EMBER GALAXY (Continuous)
  // Continuous 60fps ambient background with floating ember particles
  // ═══════════════════════════════════════════════════════════
  (function initContinuousHeroBackground() {
    ...
    // If Three.js is available, initialize WebGL particle galaxy
    if (typeof THREE !== 'undefined') {
      try {
        ...
        // Multi-cluster ember & stardust particle system
        const COUNT = 1400;
        ...
        const palette = [
          new THREE.Color('#c99a4e'), // Champagne Gold
          new THREE.Color('#e8b96e'), // Bright Gold
          new THREE.Color('#b5873d'), // Raw Brass
          new THREE.Color('#eae6e1'), // Warm Stardust
          new THREE.Color('#4fa36b'), // Muted Emerald
          new THREE.Color('#38bdf8'), // Subtle Sky Cyan
        ];
        ...
        for (let i = 0; i < COUNT; i++) {
          ...
          velocities[i3 + 1] = (Math.random() * 0.003) + 0.0008; // upward float
        ...
        function animateThree() {
          ...
          particles.rotation.y = elapsed * 0.03 + targetX * 0.35;
          particles.rotation.x = elapsed * 0.015 - targetY * 0.25;
        ...
        console.warn('WebGL init fallback to 2D canvas', e);
      }
    }

    // Fallback 2D Canvas Particle Engine (if Three.js not loaded)
    const ctx = canvas.getContext('2d');
    let width = (canvas.width = W());
    let height = (canvas.height = H());

    const particles2D = Array.from({ length: 90 }, () => ({
      x: Math.random() * width,
      ...
      color: ['#c99a4e', '#e8b96e', '#eae6e1', '#4fa36b'][Math.floor(Math.random() * 4)]
    }));

    function animate2D() {
      ctx.clearRect(0, 0, width, height);
      particles2D.forEach(p => {
        p.x += p.vx + targetX * 0.2;
        p.y += p.vy - targetY * 0.1;
